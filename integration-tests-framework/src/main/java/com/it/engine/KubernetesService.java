...
public class KubernetesService {
    ...
    public KubernetesService(ITProperties properties) {
    ...
    private void wait(KubernetesDeployment service) {
        ...
        for (FutureService dependency : dependencies) {
            try {
                ...
                throw new ITException(service.getName() + " was interrupted while waiting for " + dependency.service.getName() + " to finish deployment", e);
            } catch (ExecutionException e) {
                throw new ITException(service.getName() + " was waiting for " + dependency.service.getName() + " to finish deployment, but it threw an exception", e);
            }
        }
    }
    ...
    private void deploy(KubernetesDeployment service) {
        ...
        try {
            ...
            throw new ITException("Error in 'beforeDeploy' of " + service.getName(), e);
        }
        ...
        try {
            ...
            throw new ITException("Error in 'afterDeploy' of " + service.getName(), e);
        }
    ...
    private void healthcheck(KubernetesDeployment service) {
        ...
        while (LocalDateTime.now().isBefore(expiration)) {
            try {
                ...
                Thread.currentThread().interrupt();
                throw new ITException(service.getName() + " healthcheck was interrupted", e);
            } catch (Exception e) {
            ...
            try {
                ...
                Thread.currentThread().interrupt();
                throw new ITException(service.getName() + " healthcheck was interrupted while sleeping", e);
            }
        }
        throw new ITException(service.getName() + " failed to become healthy after " + healthcheckDuration.getSeconds() + " seconds");
    }
    ...
    public void startAndWaitToComplete(List<KubernetesDeployment> services) {
        ...
        try {
            ...
            throw new ITException("Scheduler was interrupted", e);
        } catch (ExecutionException e) {
            executorService.shutdownNow();
            throw new ITException("One of the services failed to deploy", e);
        }
    }
